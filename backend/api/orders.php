...
if ($method === 'GET') {
    ...
    if ($res['code'] === 200) {
        ...
        foreach ($wc_orders as $o) {
            // Busca CPF nos meta_data se existir
            $cpf = '';
            if (isset($o->meta_data)) {
                foreach ($o->meta_data as $meta) {
                    if ($meta->key === '_billing_cpf' || $meta->key === 'billing_cpf' || $meta->key === '_cpf') {
                        $cpf = $meta->value;
                        break;
                    }
                }
            ...
            $formatted[] = [
                ...
                'address' => [
                    ...
                    'number' => $o->billing->address_2, // Geralmente o número ou complemento
                    'city' => $o->billing->city,
