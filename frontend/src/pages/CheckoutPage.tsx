...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      try {
        ...
        if (parsed.cep) {
          setCep(parsed.cep);
          // Se tiver CEP salvo, dispara o calculo de frete
          const cleanCEP = parsed.cep.replace(/\D/g, '');
          if (cleanCEP.length === 8) fetchAddress(cleanCEP);
        }
        if (parsed.address) setRua(parsed.address);
        if (parsed.number) setNumero(parsed.number);
        if (parsed.complement) setComplemento(parsed.complement);
        if (parsed.city) setCidade(parsed.city);
  ...
  const handleCheckout = async () => {
    ...
    if (phone.replace(/\D/g, '').length < 10) {
      showToast('Por favor, informe um telefone/WhatsApp válido.', 'error');
      return;
    }
    if (!cep || cepError !== '' || freightLoading) {
  ...
  const fetchAddress = async (cleanCEP: string) => {
    ...
    try {
      ...
      if (!data.erro) {
        ...
        // Tabela de frete aproximada Correios (PAC) 2026
        const uf = data.uf;
        let freightValue = 0;

        if (uf === 'SP') {
          freightValue = 22.90; // SP Interior/Capital
        } else if (['RJ', 'MG', 'PR'].includes(uf)) {
          freightValue = 32.80; // Região 1
        } else if (['SC', 'RS', 'ES', 'DF', 'GO'].includes(uf)) {
          freightValue = 39.40; // Região 2
        } else if (['MS', 'MT', 'BA', 'PE', 'CE', 'RN', 'PB', 'AL', 'SE'].includes(uf)) {
          freightValue = 51.20; // Região 3 (Nordeste/Centro-Oeste)
        } else if (['MA', 'PI', 'PA', 'TO', 'RO'].includes(uf)) {
          freightValue = 68.90; // Região 4 (Norte/Nordeste distante)
        } else {
          freightValue = 84.50; // Extremo Norte (AM, RR, AP, AC)
        }
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full border border-premium-200 px-4 py-3 focus:outline-none focus:border-accent-dark focus:ring-1 focus:ring-accent-dark text-sm transition-all" 
              ...
              <div className="space-y-3 pt-6 border-t border-premium-100 text-sm">
                ...
                <div className="flex justify-between text-premium-600 items-center">
                  ...
                  {freightLoading ? (
                    ...
                    <span className={(freight === 0 && cep.replace(/\D/g, '').length === 8) ? "text-green-600 font-bold" : (freight === 0 ? "text-premium-400 text-xs italic" : "text-premium-900 font-medium")}>
                      {(freight === 0 && cep.replace(/\D/g, '').length === 8) ? 'GRÁTIS' : (freight > 0 ? `R$ ${freight.toFixed(2).replace('.', ',')}` : 'A calcular')}
                    </span>
                  )}
                </div>
