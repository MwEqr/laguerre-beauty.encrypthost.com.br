...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  // Revalidação de preços e estoque
  useEffect(() => {
      ...
      try {
        ...
        setItems(currentItems => {
          ...
          const updatedItems = currentItems.filter(item => {
            ...
            // 1. Produto não existe mais ou saiu de estoque
            if (!product) {
              showNotification(`O produto "${item.name}" não está mais disponível e foi removido.`, 'error');
              hasChanges = true;
              return false;
            }
            ...
            if (product.price !== item.price) {
              showNotification(`O preço de "${item.name}" foi atualizado para R$ ${product.price.toFixed(2).replace('.', ',')}.`, 'success');
